...
An automated e-commerce catalog tool transforming dry manufacturer product specifications into compelling, benefit-driven marketing copy.

---

## Key Features

- **Specification-to-benefit**: translation converting technical specs into customer value
- **Multi-tone**: copy generation (Luxury, Technical, Energetic, Minimalist)
- **Bulleted**: feature highlights optimized for mobile e-commerce shoppers
- **E-commerce**: SEO metadata (Title tag, Meta description, Alt tags)
- **Shopify**: and WooCommerce compatible JSON schema output
...
```mermaid
flowchart LR
    Specs([Raw Technical Specifications]) --> Translator[Feature-to-Benefit Engine]
    Translator --> ToneAdapter[Tone & Brand Voice Adapter]
    ToneAdapter --> Copy[E-Commerce Listing Package]
```
...
├── tests/
│   ├── __init__.py
│   └── test_product_description.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/product-description-generator.git
...
|---|---|---|
| `POST` | `/generate-description` | Transform raw product attributes into compelling product copy |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/generate-description -H "Content-Type: application/json" -d '{"product_name": "ErgoDesk Pro", "specs": {"height_range": "60-125cm", "motors": "Dual German Motors", "weight_cap": "120kg"}}'
```
...
```bash
python -m unittest tests/test_product_description.py
```
